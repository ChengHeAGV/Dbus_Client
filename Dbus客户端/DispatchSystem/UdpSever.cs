    ...
    class UdpSever
    {
                                            ...
                                            switch (Buf[7])//功能码
                                            {
                                                case 1:
                                                    ...
                                                    //{
                                                        ...
                                                        sendbyte[4] = 0x02;//帧类型
                                                        sendbyte[5] = (byte)(DstAddress >> 8);//目标地址高
                                                        sendbyte[6] = (byte)(DstAddress);//目标地址低
                                                        sendbyte[7] = 0x01;//功能码
                                                        sendbyte[8] = Buf[8];//寄存器地址高
                                                        sendbyte[9] = Buf[9];//寄存器地址低

                                                        sendbyte[10] = (byte)(Register[DstAddress, (Buf[8] << 8 | Buf[9]), 0] >> 8);//数据高
                                                        sendbyte[11] = (byte)(Register[DstAddress, (Buf[8] << 8 | Buf[9]), 0]);//数据低

                                                        int crcRes = CRC.crc_16(sendbyte, 12);

                                                        sendbyte[12] = (byte)(crcRes >> 8);
                                                        sendbyte[13] = (byte)(crcRes);
                                                        sendToUdp(ipEndPoint, sendbyte);
                                                    //}
                                                    //else
                                                    //{
                                                ...
                                                case 2:
                                                    ...
                                                    //{
                                                        ...
                                                        {
                                                            //写入寄存器
                                                            Register[DstAddress, RegAddress, 0] = (UInt16)(Buf[10] << 8 | Buf[11]);
                                                            //更新时间戳
                                                            Register[DstAddress, RegAddress, 1] = DateTimeToStamp(DateTime.Now);
                                                            sendbyte[8] = 1;//结果
                                                        }
                                                        else
                                                        {
                                                ...
                                                case 3:
                                                    ...
                                                    //{
                                                        ...
                                                        for (int i = 0; i < Num; i++)
                                                        {
                                                            sendbyte[11 + i * 2] = (byte)(Register[DstAddress, RegStartAdd + i, 0] >> 8);//数据高
                                                            sendbyte[12 + i * 2] = (byte)(Register[DstAddress, RegStartAdd + i, 0]);//数据低
                                                        }

                                                         crcRes = CRC.crc_16(sendbyte, 11 + 2 * Num);
                                                ...
                                                case 4://
                                                    ...
                                                    int RegStartAddress = Buf[8] << 8 | Buf[9];
                                                        ...
                                                        if ((RegStartAddress + Num) <= RegisterNum)
                                                        {
                                                            ...
                                                            {
                                                                //写入寄存器
                                                                Register[DstAddress, RegStartAddress + i, 0] = (UInt16)(Buf[11 + i * 2] << 8 | Buf[12 + i * 2]);
                                                                //更新时间戳
                                                                Register[DstAddress, RegStartAddress + i, 1] = DateTimeToStamp(DateTime.Now);
                                                            }
                                                            sendbyte[8] = 1;//结果
                                                        }
                                                        else
                    ...
                    for (int i = 0; i < RESPONSE_MAX_LEN; i++)
                    {
                        if (ResponseBuf[i, 0] != 0)
                        {
                            if (((ResponseBuf[i, 1] << 8) | ResponseBuf[i, 2]) == frameID)
                            {
                                ...
                                //更新寄存器
                                UdpSever.Register[DeviceAdress, RegisterAddress, 0] = Data;
                                //更新时间戳
                                UdpSever.Register[DeviceAdress, RegisterAddress, 1] = UdpSever.DateTimeToStamp(DateTime.Now);
                                return msg;
                            }
                        }
                    }
                    ...
                    for (int i = 0; i < RESPONSE_MAX_LEN; i++)
                    {
                        if (ResponseBuf[i, 0] != 0)
                        {
                            if (((ResponseBuf[i, 1] << 8) | ResponseBuf[i, 2]) == frameID)
                            {
                                ...
                                {
                                    //更新寄存器
                                    UdpSever.Register[DeviceAdress, RegisterAddress + t, 0] = Data[t];
                                    //更新时间戳
                                    UdpSever.Register[DeviceAdress, RegisterAddress + t, 1] = UdpSever.DateTimeToStamp(DateTime.Now);
                                }
        ...
        public static void sendToUdp(EndPoint EndPort, byte[] str)
        {
            if (str != null && EndPort != null)
            {
                ...
                //socket1.Bind(new IPEndPoint(ipEndPoint.Address, port));
                //socket
                socket.SendTo(buf, new IPEndPoint(ipaddress,18666));
                //socket1.Dispose();
